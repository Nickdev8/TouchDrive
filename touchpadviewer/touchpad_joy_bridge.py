...
# Revised to support single-finger throttle set-and-hold and to emit left+right
# finger state to a JSON file for Godot UI. Linux evdev/uinput remains primary.
    ...
    try:
        for event in dev.read_loop():
            if event.type == ecodes.EV_ABS:
                ...
                now = time.monotonic()
                # Single-finger vertical drag sets throttle and holds until moved again.
                if len(right_fingers) == 1:
                    f = right_fingers[0]
                    v = (f["y"] - abs_y.min) / max(1.0, (abs_y.max - abs_y.min))
                    if throttle_last_avg is None:
                        # initialize but don't immediately change throttle until movement
                        throttle_last_avg = v
                    # if moved enough, update throttle and lock until moved again
                    if abs(v - throttle_last_avg) > 0.01:
                        new_throttle = max(-1.0, min(1.0, 1.0 - 2.0 * v))
                        throttle = new_throttle
                        last_throttle = throttle
                        throttle_last_avg = v
                        throttle_mode_until = now + 5.0
                    else:
                        if now < throttle_mode_until:
                            throttle = last_throttle
                        else:
                            throttle = last_throttle
                elif len(right_fingers) >= 2:
                    if not lock_active:
                    ...
                    if now < throttle_mode_until:
                        gear = last_gear
                    # When a single finger is present, detect gear candidate from grid position too
                    if len(right_fingers) == 1:
                        f = right_fingers[0]
                ...
                # Write left+right state for Godot HUD
                if state_path and now - last_state_write > 0.02:
                    ...
                    right_active = len(right_fingers) > 0
                    right_two = len(right_fingers) > 1
                    r1x = r1y = r2x = r2y = 0.0
                    if len(right_fingers) >= 1:
                        r1 = right_fingers[0]
                        r1x = (r1["x"] - right_min_x) / max(1.0, (right_max_x - right_min_x))
                        r1y = (r1["y"] - abs_y.min) / max(1.0, (abs_y.max - abs_y.min))
                        r1x = max(0.0, min(1.0, r1x))
                        r1y = max(0.0, min(1.0, r1y))
                    if len(right_fingers) >= 2:
                        r2 = right_fingers[1]
                        r2x = (r2["x"] - right_min_x) / max(1.0, (right_max_x - right_min_x))
                        r2y = (r2["y"] - abs_y.min) / max(1.0, (abs_y.max - abs_y.min))
                        r2x = max(0.0, min(1.0, r2x))
                        r2y = max(0.0, min(1.0, r2y))
                    try:
